...
api.get_users = function(db, cb) {
  db.collection('entires').distinct('name', function(e, d) {
...
api.upsert_entry = function(req, res, db, cb) {
  ...
  db.collection('entries').update(
    q1, update, function(e, d) {
      if (d == 0) { // guess doesn't already exist
        ...
        console.log('found entry and updated');
        res.redirect('/');
        cb(e, d);
      }
    }
